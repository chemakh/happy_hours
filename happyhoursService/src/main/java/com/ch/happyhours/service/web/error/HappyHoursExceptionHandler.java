...
@ControllerAdvice
public class HappyHoursExceptionHandler {

    private final static Logger logger = LoggerFactory.getLogger(HappyHoursExceptionHandler.class);
    ...
    public ErrorDTO processDataIntegrityViolationException(DataIntegrityViolationException ex) {
        ErrorDTO errorDTO = new ErrorDTO(null, ErrorConstants.ERR_DATA_INTEGRITY_VIOLATION, ex.getMessage());
        if (ex.getCause() instanceof ConstraintViolationException) {
            ConstraintViolationException cause = ((ConstraintViolationException) ex.getCause());

            FieldErrorDTO fieldError = new FieldErrorDTO(cause.getConstraintName(), cause.getConstraintName(), String.join(": ", cause.getMessage(), cause.getSQLException().getMessage()));
            errorDTO.add(fieldError);
    ...
    @ExceptionHandler(HappyHoursException.class)
    @ResponseBody
    public ResponseEntity<ErrorDTO> processHappyHoursException(HappyHoursException ex) {
    ...
    public ResponseEntity<ErrorDTO> processFeignException(HystrixRuntimeException ex) {
        ...
        if (ex.getCause() instanceof FeignException) {
            ...
            ErrorDTO errorDTO = new ErrorDTO(null, ErrorConstants.ERR_SECURITY, description);
            ...
            return ResponseEntity.badRequest().body(new ErrorDTO(null, ErrorConstants.ERR_INTERNAL_SERVER_ERROR, ex.getMessage()));
    }
